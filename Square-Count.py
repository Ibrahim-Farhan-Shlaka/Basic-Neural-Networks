...
def randomMatrix(COUNT):
    ...
    for i in range(COUNT):
        squareCount = np.random.randint(2, 10)
        y[i] = squareCount
        
        for j in range(squareCount):
            #random centers and sizes for the sqaures
            xCenter = np.random.randint(0, 25)
            yCenter = np.random.randint(0, 25)
            ...
            #drawing the top and bottom lines of the square
            for x in [xMin, xMax]:
                if 0 <= x < 25:
                    yStart = max(0, yMin)
                    yEnd = min(24, yMax)
                    X[i, x, yStart:yEnd+1] = 1
            
            #drawing left and right lines of the square
            for YDraw in [yMin, yMax]:
                if 0 <= YDraw < 25:
                    xStart = max(0, xMin + 1)
                    xEnd = min(24, xMax - 1)
                    X[i, xStart:xEnd+1, YDraw] = 1
...
plt.xlabel("True amount of squares")
plt.ylabel("Predicted amount of squares")
plt.title("True vs Predicted amount of squares")
plt.legend()
plt.grid()
plt.savefig(f'Basic-Neural-Networks/E/{COUNT}samples_{EPOCHS}epoch_{BATCH}batch_{TEST}test.jpg', bbox_inches='tight')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'Basic-Neural-Networks/E/{COUNT}samples_{EPOCHS}epoch_{BATCH}batch_{TEST}test LOSSES.jpg', bbox_inches='tight')
plt.show()
