...
def randomMatrix(COUNT):
    ...
    for i in range(COUNT):
        ...
        X[i, point2[0], point2[1]] = 1
        #calculating distance between them and saving it, this is the label for the neural network
        y[i] = np.sqrt((point1[0] - point2[0])**2 + (point1[1] - point2[1])**2)
...
plt.legend()
plt.grid()
plt.savefig(f'Basic-Neural-Networks/A/A_{COUNT}samples_{EPOCHS}epoch_{BATCH}batch_{TEST}test.jpg', bbox_inches='tight')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'Basic-Neural-Networks/A/A_{COUNT}samples_{EPOCHS}epoch_{BATCH}batch_{TEST}test LOSSES.jpg', bbox_inches='tight')
plt.show()
