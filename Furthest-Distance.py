...
def randomMatrix(COUNT):
    ...
    for i in range(COUNT):
        ...
        #generatg random points and setting them in matrix
        ...
        #calculating distances, finding maximum and using that as a label
        max = 0
        for j in range(pointCOUNT):
            for k in range(j+1, pointCOUNT):
                dx = points[j][0] - points[k][0]
                dy = points[j][1] - points[k][1]
                if np.sqrt(dx**2 + dy**2) > max:
                    max = np.sqrt(dx**2 + dy**2)
        #we put the maximum distance we just found here
        y[i] = max
...
plt.xlabel("True max Distance")
plt.ylabel("Predicted max Distance")
